...
function preload() {
    var assets = {
        spritesheet: {
            birdie: ['assets/birdiemobile.png', 20, 20],
        ...
        image: {
            finger: ['assets/fingermobile.png'],
...
function reset() {
	...
	gameOverText.renderable = false;    birdie.body.allowGravity = false;
    birdie.angle = 0;
    birdie.reset(game.world.width / 5, game.world.height / 2);
...
function spawnCloud() {
    ...
    cloud.body.velocity.x = -300 / cloudScale;
    cloud.anchor.y = 0;
...
function o() {
    return 105 + 60 * ((score > 50 ? 50 : 50 - score) / 50);
}
...
function spawnFingers() {
    ...
    inv.body.velocity.x = -300;

    fingersTimer.start();
    fingersTimer.add(1 / (1 / 1));
}
...
function setGameOver() {
    ...
    var mesg = (score <= 0 ? "Zero!" : (score < 5 ? "Oh Dear!" : (score < 15 ? "Alright!" : (score < 20 ? "Not Bad!" : (score < 25 ? "Better!" : (score < 30 ? "Nice!" : (score < 40 ? "Such Flap!" : (score < 50 ? "Very WOW!" : (score < 65 ? "Impressive!" : (score < 80 ? "Unbelievable!" : "Godlike!"))))))))));
...
function update() {
    if (gameStarted) {
        ...
        if (gameOver) {
            ...
            gameOverText.angle = Math.random() * 3 * Math.cos(game.time.now / 100);*/
        } else {
            // Check game over
    ...
    // Scroll fence
    if (!gameOver) {
        fence.tilePosition.x -= game.time.physicsElapsed * 300 / 2;
    }
}
