...
function preload() {
    var assets = {
        spritesheet: {
            birdie: ['assets/birdiedoge.png', 24, 24],
            clouds: ['assets/cloudsdoge.png', 128, 64],
...
function setGameOver() {
    ...
    if(!name || name == "null" || name == "Such name, please enter." || name.length > 10 || name.length == 0){
      name = prompt("Such score! Please enter your name.");
      while(name && (name.length > 10 || name.length == 0))
        name = prompt("Many Error! Must be less than 10 characters.", name);
      if(name) {
    ...
    var mesg = (score <= 0 ? "Such Sadness!" : (score < 5 ? "Many Bad!" : (score < 15 ? "Such Average!" : (score < 20 ? "Many OK!" : (score < 25 ? "Much Alright!" : (score < 30 ? "So Wings!" : (score < 40 ? "Such Flap!" : (score < 50 ? "Very WOW!" : (score < 65 ? "Many Score!" : (score < 80 ? "Such Excite!" : "Jeebus!"))))))))));
